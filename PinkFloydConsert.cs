...
{
    public partial class PinkFloydConsert : Form
    {
        private Panel[] seats = new Panel[30];
        char[] seat_status = new char[30];

        public PinkFloydConsert()
        {
        ...
        public void update_db(char new_seat_status, int seat_id)
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                try
                {
                    ...
                    string updateSql = $"UPDATE Pinkfloyd_concert SET seat_status = '{new_seat_status}' WHERE seat_id = {seat_id};";
        ...
        public void get_initial_data_db()
        {
            ...
            using (SQLiteConnection connection = new SQLiteConnection(connectionString))
            {
                try
                {
                    ...
                    string query = $"SELECT seat_id, seat_status FROM Pinkfloyd_concert;";
        ...
        private void PinkFloydConsert_FormClosing(object sender, FormClosingEventArgs e)
        {
            Application.Exit();
        }
